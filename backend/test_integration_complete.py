...
def test_file_upload_and_conversion():
    ...
    try:
        ...
        print(f"  ❌ File upload and conversion failed: {e}")
        return None
...
def test_audio_download(job_id):
    ...
    try:
        ...
        print(f"  ✅ Audio file downloaded successfully ({len(response.content)} bytes)")
        return True
    except Exception as e:
...
def test_conversions_list():
    ...
    try:
        ...
        print(f"  ✅ Conversions list retrieved ({len(data['data'])} conversions)")
        return True
    except Exception as e:
        print(f"  ❌ Conversions list failed: {e}")
        return False
...
def test_voice_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                print(f"  ✅ Voices list retrieved ({data.get('total_voices', 0)} voices)")
            else:
                print("  ⚠️  Voices endpoint returned unsuccessful response")
        else:
            print(f"  ⚠️  Voices endpoint returned status {response.status_code}")
        
        return True
    except Exception as e:
        print(f"  ⚠️  Voice endpoints test failed: {e}")
